...
const Subject = () => {
    ...
    return ( <
        ...
        <
        /div>{" "} <
        h1 className = "SSheading" > { " " } { params.class + " - Section " } { params.section } { " " } <
        /h1>{" "} <
        div className = "createschool" >
        <
        button onClick = {
            () => setCreateClass(true) } > Create Subject < /button>{" "} <
        /div>{" "} <
        div className = "adhmschools" > { " " } {
            subjects.map((data, i) => ( <
                ClassCard choose = "subjects"
                class = { data.Subject }
                key = { i }
                />
            ))
