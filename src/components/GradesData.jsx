...
const updateclassstatus=async(e)=>{
    ...
    try{
      ...
      const{data} =await http.post(`/createresult?token=${currentUser?.token}`,{
    ...
    <form  style={{backgroundColor:"white",padding:"2rem 1.5rem",borderRadius:".5rem",width:"35%"}} onSubmit={updateclassstatus}>
    <Typography color={"black"} variant="h4" align="center" gutterBottom>
      ...
      <Grid item xs={12}>
        ...
        >
            {
              enrollments?.map((item,i)=>(
                    <MenuItem key={i} value={item.id}>{item?.student.name}</MenuItem>
                ))
            }
        ...
        >
            {
              ["A","A-","B+","B","B-","C+","C","C-","D","F","I"].map((item,i)=>(
        ...
        <Grid item xs={12} sx={{marginTop:"1rem"}}>
          <Button disabled={subject===''||enrollmentId===''}  type="submit" variant="contained" color="success">
            Add Result
          </Button>
        </Grid>
