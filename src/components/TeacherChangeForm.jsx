...
const updateclassstatus=async(e)=>{
    ...
    try{
        //
      const{data} =await http.put(`/changeteacher?token=${currentUser?.token}`,{
           id:classId,
           teacherid:teacherId
        })
        alert(data?.message)
    ...
    <form  style={{backgroundColor:"white",padding:"2rem 1.5rem",borderRadius:".5rem"}} onSubmit={updateclassstatus}>
    <Typography color={"black"} variant="h4" align="center" gutterBottom>
      ...
      <Grid item xs={12}>
        ...
        >
            {
                classes?.map((item,i)=>(
                    <MenuItem key={i} value={item.id}>{item?.classname + item?.section}</MenuItem>
                ))
            }
